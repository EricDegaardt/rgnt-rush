...
const Game = () => {
  ...
  return <div className="relative bg-black w-full overflow-hidden" style={{
    ...
    aspectRatio: '3 / 4'
            ...
            {collectionEffects.map(effect => <CollectionEffect key={effect.id} x={effect.x} y={effect.y} onComplete={() => handleEffectComplete(effect.id)} />)}
            {splashEffects.map(effect => <SplashEffect key={effect.id} x={effect.x} y={effect.y} onComplete={() => handleSplashComplete(effect.id)} />)}
